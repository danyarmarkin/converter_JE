...
public class JpgQualityTextField extends JTextField {
    SessionsLib mSessionsLib;

    public JpgQualityTextField(SessionsLib sessionsLib) {
        mSessionsLib = sessionsLib;

        for (Session session : mSessionsLib) {
            for (Camera camera : session.getCameras()) setText(String.valueOf(camera.getJpgQuality()));
        }
